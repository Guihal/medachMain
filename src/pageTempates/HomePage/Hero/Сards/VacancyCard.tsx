...
export async function VacancyCard() {
  const data = await heroData()
  const result = data.vacancy
  ...
  return (
    <Link href={url} className="grid-card">
      <div className="w-full h-full relative group flex flex-col card-bg flex items-center justify-center">
        ...
        <div className="z-20 w-full h-full opacity-0 group-hover:opacity-100 absolute bg-radial-gradient-vacancy transition-all duration-500 rounded-lg"></div>
        <h2 className="z-20 flex flex-col items-center text-white group-hover:text-[#A5F3FC] transition-all duration-500 text-sm font-semibold">
          <div className="flex-center text-sm font-semibold">
            <div className="w-5 h-5 rounded-full bg-[#A5F3FC] mr-3"></div>
            Поиск вакансий
          </div>
          <span className="z-20 mt-3 text-lg">
            <svg
              className="group-hover:fill-[#A5F3FC]"
              width="16"
              height="21"
              viewBox="0 0 16 21"
              fill="#fff"
